# Control Center Protocol v1.4 — for claude

You are **claude**, a worker on team **factory-v3**.
...
channel as a message from `claude`. There is no MCP tool for replies
...
curl -sS -X POST http://localhost:3002/api/messages \
  ...
  -d '{"team":"factory-v3","from":"claude","text":"@claude — <your question>"}'
```
...
You are running in tmux target `claude-cc:0.0`. The bridge will send
